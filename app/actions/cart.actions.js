...
export const addProductToCart = async (userId, product) => {
  ...
  if (!user) {
    ...
    if (product.quantity < 0) {
      ...
      if (isProductInCart) {
        const existingProductIndex = user.cart.findIndex((cartProduct) => {
          return cartProduct._id.toString() === product._id.toString();
        });
...
export const removeProductFromCart = async (userId, productId, restock) => {
  await connectMongo();
  ...
  if (!user) {
    ...
    const cartProduct = user.cart.find((cartProduct) => {
    ...
    user.cart = user.cart.filter((cartProduct) => {
      return cartProduct._id.toString() !== productId;
    });
  ...
  try {
    ...
    const userUpdatePromises = users.map(async (user) => {
      user.cart = user.cart.filter(
        (cartItem) => new Date() <= new Date(cartItem.expiresAt)
      );
      await user.save();
    });
