...
import { getAuthTokenByEmail } from "./queries/token.queries";
        ...
        try {
          ...
          if (user) {
            ...
            if (isMatch) {
              ...
              await generateAuthToken(credentials.email, credentials.remember);
              // reset the login attempt if user successfully logs in
              await resetLoginAttempt(credentials.email);
              return user;
            } else {
              // implemented a max retry and jail feature
              await increaseLoginAttemptIfPasswordIsWrong(credentials.email);
      ...
      if (!token?.access_token && token?.user?.sub) {
        try {
          ...
          const accessTokenExpired =
            new Date(existingToken?.aTExp) < new Date();
